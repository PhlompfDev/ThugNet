...
---@diagnostic disable: unused-local, unused-vararg
    ...
    -- handle a right-click event (return true if consumed)
    ---@param event mouse_interaction mouse interaction event
    ---@return boolean consumed
    function protected.handle_right_click(event) return false end
...
---@diagnostic enable: unused-local, unused-vararg
    ...
    function public.remove(id)
        ...
        if protected.children[index] ~= nil then
            ...
            protected.children[index] = nil
            protected.child_id_map[id] = nil
            -- Reset next_y so that auto-placed children added after a remove do not
            -- overflow the parent window.  Containers such as ListBox that manage
            -- child positions themselves (via on_added / update_positions) will
            -- immediately reposition any newly added child, so starting from y=1
            -- is always safe here.
            self.next_y = 1
        end
    end
    ...
    -- Forwards every argument: PushButton.set_value(val, mirrored) takes a second
    -- parameter that applies visual feedback WITHOUT firing the callback, and
    -- dropping it here made that parameter unreachable from outside the element --
    -- so mirroring state into a button synthesized a real click instead.
    ---@param value any new value
    function public.set_value(...) protected.set_value(...) end
    ...
    function public.handle_mouse(event)
        if protected.window.isVisible() then
            ...
            if ini_in then
                ...
                -- snapshot children before iterating: callbacks may delete/add children mid-loop
                local _ch = {}
                for _, v in pairs(protected.children) do _ch[#_ch + 1] = v end
                for _, child in ipairs(_ch) do if child then child.get().handle_mouse(c_event_T) end end
    ...
    -- propagate a right-click to the deepest matching child; returns true if consumed
    ---@param event mouse_interaction mouse interaction event (absolute coords)
    ---@return boolean consumed
    function public.handle_right_click(event)
        if not protected.window.isVisible() then return false end
        local x, y = event.current.x, event.current.y
        if not protected.in_window_bounds(x, y) then return false end

        local event_T = events.mouse_transposed(event, self.position.x, self.position.y)
        local c_event_T = events.mouse_transposed(event_T, protected.mouse_window_shift.x + 1, protected.mouse_window_shift.y + 1)

        -- depth-first: try children before self
        for _, child in pairs(protected.children) do
            if child.get().handle_right_click(c_event_T) then
                return true
            end
        end

        -- then try this element's own handler
        return protected.handle_right_click(event_T)
    end

    -- set or replace the right-click handler for this element from outside
    ---@param fn function handler(event) -> boolean
    function public.set_right_click_handler(fn)
        protected.handle_right_click = fn
    end

    -- set or replace the left-click handler for this element from outside.
    -- Unlike right-click there is no "consumed" return: handle_mouse runs this
    -- BEFORE descending into children, and children still receive the event. Use
    -- it on a container that needs the raw click coordinates (the editor's move
    -- mode), and disable the children whose own clicks must not fire.
    ---@param fn function handler(event)
    function public.set_left_click_handler(fn)
        protected.handle_mouse = fn
    end
